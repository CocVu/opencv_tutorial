...
int main( int argc, char** argv )
{
    ...
    else
    {
        for ( size_t i = 0; i < pos_lst.size(); ++i )
        {
            if( pos_lst[i].size() != pos_image_size )
            {
                ...
                // resize(image, dst, cv::Size(150,150));
              resize(pos_lst[i],pos_lst[i], pos_image_size);
                exit( 1 );
            }
        }
    ...
    //         // cout << "All positive images should be same size!" << endl;
    //         // resize(image, dst, cv::Size(150,150));
    //         resize(neg_lst[i],neg_lst[i], pos_image_size);
    //         exit( 1 );
    //       }
    //   }
    ...
    labels.assign( positive_count, +1 ); // vector assign(7, 100) => 7 number 100 100 100 100 100 100 100 100
